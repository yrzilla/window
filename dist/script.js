...
var changeModalState = function changeModalState(state) {
  ...
  bindActionToElems('change', windowProfile, 'profile'); // windowForm.forEach((item, i) => { //При клике на определенный элемент буду записывать его в обьект стейта(modalState)
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault();
      var statusMessage = document.createElement('div'); // Создаю блок оповещения пользователя

      statusMessage.classList.add('status');
      item.appendChild(statusMessage);
      var formData = new FormData(item); // Собираю данные с формы

      if (item.getAttribute('data-calc') === 'end') {
        // Проверка формы(та ли форма)
        for (var key in state) {
          //перебираем свойства и отправляем в formData
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    // Повесил обработчик событий на область с табами
    var target = e.target;

    if (target && (target.classList.contains(tabSelector.replace(/\./, '')) || target.parentNode.classList.contains(tabSelector.replace(/\./, '')))) {
      // Проверил что кликнул на один из табов
...
var timer = function timer(id, deadline) {
  // id - куда будет рендерится таймер, deadline - до какого времени идет таймер
  ...
  var getTimeRemaining = function getTimeRemaining(endTime) {
    var t = Date.parse(endTime) - Date.parse(new Date()),
        // t - разница между дедлайном и текущей датой(время до мс)
    seconds = Math.floor(t / 1000 % 60),
        // (t/1000) - кол-во секунд
    minutes = Math.floor(t / 1000 / 60 % 60),
        // (t/1000/60) - кол-во минут которое содержится в t
    hours = Math.floor(t / (1000 * 60 * 60) % 24),
        // (t/(1000 * 60 * 60)) - кол-во часов
